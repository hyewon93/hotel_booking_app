                    ...
                    <div className="grid grid-cols-6 gap-4">
                        {existingImageUrls.map((imageUrl) => (
                            <div className="relative group" key={imageUrl}>
                                ...
                                <button onClick={(event) => handleDelete(event, imageUrl)} className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 text-white">
                                    Delete
                                </button>
                            </div>
                        ))}
